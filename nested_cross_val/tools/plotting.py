...
def plot_hyperparameters(ncv, estimator, name, discrete=True, ax=None, **kwargs):
    ...
    **kwargs
    ...
    df = pd.DataFrame(ncv.best_params_[estimator])
...
def plot_score(ncv, estimator, score_name, display, ax=None, **kwargs):
    ...
    **kwargs
    ...
    """
    X1 = ncv.outer_results_[estimator]['training_' + score_name]
    X2 = ncv.outer_results_[estimator]['test_' + score_name]
    ...
    if display == 'barplot':
        ...
        ax.legend()
        ax.set_title(score_name + " (" + estimator + ")", fontsize=14)
        ...
        ax[0].set_title(score_name + ' distribution (training)' + " (" + estimator + ")", fontsize=14)

        ax[1].hist(X2, bins='auto', color='red', **kwargs)
        ax[1].set_title(score_name + ' distribution (test)' + " (" + estimator + ")", fontsize=14)

        ax[2].hist(np.array(X1) - np.array(X2), bins='auto', color='green', **kwargs)
        ax[2].set_title(score_name + ' distribution (training - test)' + " (" + estimator + ")", fontsize=14)
    return


def plot_roc(ncv, estimator, method="predict_proba", plot_outer_folds=True, ax=None):
    """ Plot roc curves
    ...
    tprs = []
    aucs = []
    for scores, y_test in ncv.get_probas(estimator=estimator, method=method):
    ...
    ax.set_title("ROC curve" + " (" + estimator + ")", fontsize=14)

    return


def plot_PR(ncv, estimator, method="predict_proba", ax=None):
    ...
    ax.set_title("Precision-Recall curve" + " (" + estimator + ")", fontsize=14)
    return


def plot_mccF1(ncv, estimator, method='predict_proba', ax=None):
    ...
    ax.set_title("MCC_F1 curve" + " (" + estimator + ")", fontsize=14)
    return


def plot_feature_importances(df, normalize=True, display='all', imp_thr=None, stab_thr=None, handle_negatives=False,
                             ax=None):
