...
def collect_importances(ncv , importances , feature_names = None):
    ...
    temp = {}
    i = 0
    for imp in ncv.get_attributes(attribute = importances):
        temp['out fold ' + str(i)] = pd.Series(imp , index = feature_names)
        i+=1
    return pd.DataFrame(temp)

def collect_meta_importances(ncv , importances , components ,  projections = None , full_components = None , full_projections = None 
                             , clustering_space = "features" , consensus = "centrotypes" ,  plot = False , plot_method = "mds" , ax = None):
    ...
    L = [x for x in ncv.get_attributes(attribute = components)]
    ...
    if clustering_space == "features" :
        ...
        L = [x.T for x in ncv.get_attributes(attribute = projections)]
        if full_projections is not None :
            L.append(full_projections.T)
        Sim = np.abs(np.corrcoef(x=np.vstack(L), rowvar=True))
    
    clustering = AgglomerativeClustering(n_clusters = n_components , affinity = "precomputed"  , linkage = 'average').fit(1 - Sim)      
    ...
    if consensus == "centrotypes" :
        ...
        Consensus = pd.DataFrame(full_components , index = ['cluster ' + str(j) for j in clustering.labels_[- full_components.shape[0] :]])       
...
def _projection(Sim , cluster_labels , method = "mds" , specific_idx = None , ax = None):
    ...
    if specific_idx is not None :
        ...
        ax.scatter(P[other_idx  , 0] , P[other_idx  , 1] , c=cluster_labels[other_idx] , cmap = 'viridis' , label = "ncv components")
